...
import { getStyles } from '../../computeds/getStyles';

import useLongPress from '../../utils/hooks/useLongPress';

import { ItemTypes } from '../../data/dd-item-types';

type DDHandleSectionProps = {
    left?: number;
    top?: number;
    subTitle?: string;
    ratio?: number[];
...
export default function DDHandleSection(props: DDHandleSectionProps) {
    ...
    const uniqueID = `app-builder-draggable__item-grid` + useId().replace(/\:/g, "-");
    const _left = typeof left === 'undefined' ? 0 : left;
    const _top = typeof top === 'undefined' ? 0 : top;
    const _sectionArgs = typeof sectionArgs === 'undefined' ? null : sectionArgs;

    const _renderComponent: any = (preview: boolean = false) => {

        if (!preview) {
        return <div className={`app-builder-draggable-view app-builder-draggable-view--section ${targetShow}`}>
            {/* PREVIEW SCREENSHOT */}
            <div className="row">
                {ratio?.map((item: any, i: number) => {
                    const [colNum, colClassName] = item;
                    return <div key={'col-' + i} data-grid={`${colClassName}`} className={`col-${colNum} app-builder-draggable-view__column`}></div>
                })}
            </div>
        </div>;
        } else {
            ...
            <div className="app-builder-draggable" id={uniqueID}>
                ...
                <div className="row">
                    <div className="col-auto">
                        <div className="input-group input-group-sm mb-3 app-builder-draggable-con__wrapper">
                            <span className="input-group-text">{subTitle}</span>
                            <input type="text" className="form-control" value={ratio?.join(' ')} />
                        </div>
                    </div>
                    <div className="col-auto app-builder-draggable-handle__wrapper">
                        <span 
                            ...
                            ref={drag} 
                            {...longPressEvent} 
                            style={getStyles(_left, _top, isDragging)}
                        >
